...
cp -R build/dependencyUpdates/. deploy-stuff/Skeleton
cp -R build/docs/javadoc/. deploy-stuff/Skeleton/javadoc
cp -R build/reports/. deploy-stuff/Skeleton
cp -R build/libs/. deploy-stuff/Skeleton

# create mvn repo
mkdir deploy-stuff/mvn-repo/
mvn deploy:deploy-file -Dfile=build/libs/Skeleton-1.0-SNAPSHOT.jar -DpomFile=pom.xml  -Durl=file://${TRAVIS_BUILD_DIR}/deploy-stuff/mvn-repo
